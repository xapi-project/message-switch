...
let get_ok = function
  | Result.Ok x ->
      x
  | Result.Error _ ->
      failwith "Expecting OK, got Error"

module Lwt_result = struct let ( >>= ) m f = m >>= fun x -> f (get_ok x) end
...
let make_server config =
  ...
  let save statedir qs =
    ...
    with_file temp_path [Unix.O_CREAT; Unix.O_TRUNC; Unix.O_WRONLY] 0o600
      (fun fd ->
        ...
        Lwt_io.write_from_exactly oc txt 0 n >>= fun () -> Lwt_io.flush oc)
  ...
  let load statedir =
    ...
    >>= fun () -> return (Q.queues_of_sexp (Sexplib.Sexp.of_string txt))
  in
  let module Redo_log =
  ...
  let redo_log =
    ...
    | Some statedir ->
        ...
        (Block.connect ~buffered:true redo_log_path >>= function
         | `Ok block ->
             Lwt.return block
         | `Error (`Unknown s) ->
             Lwt.fail
               (Failure
                  (Printf.sprintf "Unexpected failure when opening block: %s" s))
         | `Error _ ->
             Lwt.fail (Failure "Other error"))
        >>= fun block ->
