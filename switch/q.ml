...
let contents q =
  Internal.(Int64Map.fold (fun i e acc -> ((q.name, i), e) :: acc) q.q [])

module Directory = struct
  let add queues ?owner name = Op.Directory (Op.Add (owner, name))

  let remove queues name = Op.Directory (Op.Remove name)
  ...
  let messages =
    List.map
      (fun name ->
        ...
        Int64Map.fold
          (fun id e acc -> ((name, id), e.Protocol.Entry.message) :: acc)
          not_seen [])
      names
  in
