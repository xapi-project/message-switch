...
module CompactMessage : sig
  ...
  val of_message : Protocol.Message.t -> t

  val to_message : t -> Protocol.Message.t
  ...
  let rec truncate_rpc =
    let open Rpc in
    function
    | (Int _ | Int32 _ | Bool _ | Float _ | DateTime _ | Null) as orig ->
        orig
    | Enum l ->
        Enum (List.rev_map truncate_rpc l |> List.rev)
    | Dict d ->
        Dict (List.rev_map truncate_rpc_kv d |> List.rev)
    | String s ->
        String (truncate s)
...
module CompactEvent : sig
  ...
  val of_event : Protocol.Event.t -> t

  val to_event : t -> Protocol.Event.t
end = struct
  open Protocol
  ...
  let of_event t =
    ...
    ; message= of_message t.Event.message
    ; processing_time= t.Event.processing_time
    }
end
...
let get buffer from timeout :
    (int64 * Protocol.Event.t) list Lwt.t =
